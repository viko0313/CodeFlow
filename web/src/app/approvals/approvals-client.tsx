    ...
    <div className="mx-auto grid max-w-[1480px] gap-5 px-4 py-5">
      <section className="rounded-lg border border-[var(--line)] bg-white p-5">
        <div className="mb-4 flex items-center justify-between">
          ...
          <Badge>{pending.data?.length ?? 0} pending</Badge>
        </div>
        <div className="grid gap-3">
          {(pending.data ?? []).map((item) => {
            ...
            return (
              <div key={item.id} className="rounded-lg border border-[var(--line)] p-4">
                ...
                {item.command ? <Row label="Command" value={item.command} /> : null}
                {item.path ? <Row label="Path" value={item.path} /> : null}
                {item.request_id ? <Row label="Request ID" value={item.request_id} /> : null}
          ...
          {!pending.data?.length ? <p className="text-sm text-[var(--muted)]">No pending approvals.</p> : null}
        </div>
      </section>

      <section className="grid gap-5 lg:grid-cols-2">
        <div className="rounded-lg border border-[var(--line)] bg-white p-5">
          <h2 className="mb-3 text-lg font-semibold">Recent Approvals</h2>
        ...
        <div className="rounded-lg border border-[var(--line)] bg-white p-5">
          <h2 className="mb-3 text-lg font-semibold">Approval Events</h2>
          <div className="grid gap-2">
            {(events.data ?? [])
